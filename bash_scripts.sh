...
alias savepower='sudo powertop --auto-tune;xinput disable $(xinput list | grep Touchscreen | perl -pe "s/.*id=(\d+).*/\1/");sudo tee /proc/acpi/ibm/bluetooth <<< disabled >/dev/null;sudo tee /sys/class/backlight/intel_backlight/brightness <<< 426 >/dev/null'
